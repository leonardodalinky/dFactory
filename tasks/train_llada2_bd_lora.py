...
class LLaDA2ModelArguments(ModelArguments):
    ...
    lora_alpha: float = field(
        default=32.0,
        metadata={"help": "The weight/scaling factor of the LoRA update matrices."},
    )
    ...
    pretrained_lora_path: Optional[str] = field(
        default=None,
        metadata={"help": "Pretrained LoRA path for resume."},
    )
...
class LLaDA2TrainingArguments(TrainingArguments):
    train_architecture: Literal["lora", "full"] = field(
        default="full",
        metadata={"help": "LoRA training or full training."},
    )
    beta1: float = field(
        default=0.9,
...
def main():
    ...
    if args.train.train_architecture == "lora":
        ...
        logger.info_rank0(f"LoRA trainable params: {trainable:,} / {total:,} ({100 * trainable / total:.2f}%)")
